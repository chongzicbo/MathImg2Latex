...
# 导出模型为 ONNX 格式
def export_to_onnx(model, onnx_path, dummy_input, decoder_input_ids):
    model.eval()
    with torch.no_grad():
        torch.onnx.export(
            model,
            ...
            input_names=["input", "decoder_input_ids"],
            output_names=["output"],
            dynamic_axes={
...
# 导出模型
export_to_onnx(model, onnx_path, dummy_input, decoder_input_ids)
