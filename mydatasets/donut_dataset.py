...
nougut_train_transform = alb.Compose(
    [
        alb.RGBShift(r_shift_limit=15, g_shift_limit=15, b_shift_limit=15, p=0.15),
        alb.Compose(
            [
        ...
        alb.RandomBrightnessContrast(0.1, (-0.2, 0), True, p=0.2),
        alb.ImageCompression(95, p=0.15),
        alb.GaussNoise(2, p=0.2),
...
class NougatDataset(Dataset):
    def __init__(
        ...
        **kwargs
        ...
        all_images = [
            path for images in data_root for path in glob.glob(join(images, "*.png"))
        ]
...
class NougatPadFixSizeCollectFn(object):
    ...
    def __call__(self, instances):
        ...
        batch["labels"] = pad_sequence(
            [torch.LongTensor(x) for x in batch["labels"]],
            batch_first=True,
            padding_value=self.pad_token_id,
        )
        # attention_mask
        batch["mask"] = pad_sequence(
            [torch.LongTensor(x) for x in batch["mask"]],
            batch_first=True,
            padding_value=0,
        )
        pixel_values = []
